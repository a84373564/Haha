...
if not performance_path.exists():
    raise FileNotFoundError("找不到績效資料 king_performance.json")
perf = json.loads(performance_path.read_text())

# 載入記憶與市況資料（非必要）
memory = json.loads(memory_path.read_text()) if memory_path.exists() else {}
market = json.loads(market_status_path.read_text()) if market_status_path.exists() else {}
...
# 懲罰失敗指標
fail_indicators = memory.get("fail_indicators", [])
